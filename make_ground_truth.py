...
def make_ground_truth(dataset_name: str, split: str):
    ...
    lower_case_dataset_name = dataset_name.lower().replace("-", "_")
    os.makedirs(f"exp/ground_truth/{lower_case_dataset_name}", exist_ok=True)
    with open(f"exp/ground_truth/{lower_case_dataset_name}/{split}.json", "w") as f:
